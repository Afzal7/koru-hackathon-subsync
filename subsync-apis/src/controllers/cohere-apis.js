...
router.post("/summarize", async (req, res) => {
  try {
    ...
    res.json({
      summary: response.message.content[0].text || "No summary generated",
    });
  } catch (error) {
...
async function summarizeNotes(notes) {
  ...
  const response = await cohere.chat({
    ...
    messages: [
      {
        role: "user",
        content: `Summarize these student notes briefly for a substitute teacher:\n${notes.join(
          "\n"
        )}`,
      },
    ],
...
async function rankStudents(students) {
  ...
  return studentRankingResponse.results.map(
    (item) => studentsWithRankedNotes[item.index]
  );
}
